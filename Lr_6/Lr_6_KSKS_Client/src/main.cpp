...
void loop() {
  ...
  day = day - y * 365;
  h = h - day * 24 - y * 365 * 24;
  m = m - day * 24 * 60 - y * 365 * 24 * 60 - h * 60;
  sec = sec - day * 24 * 60 * 60 - y * 365 * 24 * 60 * 60 - (h * 60 * 60) - (m * 60);
  String str = convert(h) + "." + convert(m) + "." + convert(sec);
  char text[str.length() + 1];
  str.toCharArray(text, str.length() + 1);
  display.drawText(15, 15, 54, RGB(255, 255, 255), text);
  delay(1000);
}